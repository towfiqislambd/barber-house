...
const AllCategories = ({ categoryData }) => {
    return (
        <div>
            <h3 className="font-outfit text-xl font-medium text-[#2C2C2C] mb-3">Spa</h3>
            ...
            <div className="space-y-8">
                {
                    categoryData.map(data => <div key={data.id}>
                        <h3 className="font-outfit text-xl font-medium text-[#2C2C2C] mb-3">{data.category}</h3>
                        <div className="space-y-4">
                            {
                                data.info.map((infoData, idx) => <div key={idx} className="bg-white rounded-xl border-l-[5px] border-[#8AE3E7] py-5 3xl:py-8 3xl:px-5 px-3 shadow-[0px_0px_4px_0px_rgba(4,0,116,0.10)] flex justify-between items-center">
                                    <div className="">
                                            ...
                                            <PopoverContent className="w-[210px] absolute right-0 py-7 px-5">
                                                <div className="space-y-6 flex flex-col text-left items-start">
                                                    ...
                                                    <Link to='/businessDashboard/editService'>
                                                        <button className="text-[#545454] font-medium hover:text-blue-500">Edit service</button>
                                                    </Link>
                                                    <Link to='/businessDashboard/addService'>
                                                        <button className="text-[#545454] font-medium hover:text-blue-500">Add service</button>
                                                    </Link>
