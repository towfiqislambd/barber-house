...
const LeftSideCard = ({ store }) => {
  return (
    <Link to={`/shop-info/${store?.id}`} className={`flex flex-col w-full`}>
      {/* Card Image */}
      <div className=" h-[230px] 3xl:h-[260px] w-full">
        <img
      ...
      <div className="border 2xl:p-4 xl:p-6 p-2 rounded-b-xl border-b-[0.5px] border-l-[0.5px] border-r-[0.5px] border-borderColorLight bg-white">
        ...
        <div className="xl:space-y-4 space-y-2 lg:mt-5 mt-2">
          {store?.store_services.map((service, index) => (
            <div
              key={index}
