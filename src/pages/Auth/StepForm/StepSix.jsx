import { LeftSideArrowSvg, UploadSvg } from "@/components/svgContainer/SvgContainer";
...
const StepSix = ({ step, setStep, setFormData }) => {
  ...
  const onSubmit = () => {
    ...
    // Data
    if (isValid) {
      setFormData(prevData => ({ ...prevData, uploadedFiles }))
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div id="dragger" className="max-w-[550px] mx-auto space-y-5">
        ...
        {[
          ...
          { name: "bankStatement", label: "Business Bank Account Statement (Last 3 Months)" }
        ].map(({ name, label }) => (
          <div key={name}>
              ...
              <label className="border cursor-pointer rounded p-5 flex justify-between items-center">
                <div>
                  <div className="flex gap-2 items-center mb-2">
                    <UploadSvg />
                    <p className="text-[#545454] font-outfit">Drag and drop files here or upload</p>
                  </div>
