    ...
    <div>
      ...
      <div className="flex gap-[30px]">
        <div className="flex-shrink-0 w-[814px]">
          <div className="border-borderColorLight border rounded-[16px] py-[48px] px-[50px]">
            <div className="flex flex-col gap-[20px]">
              {memberships.map(member => (
                <div
                  className="flex flex-col gap-[20px]  bg-primary-gradient rounded-[16px] p-[32px] cursor-pointer"
                  key={member.id}
                >
                  <div className="py-[6px]">
                    <p className="flex gap-[10px] items-center text-white font-manrope font-medium text-[18px]">
                      <span className="">
                        <FaCalendarAlt />
                      </span>
                      {member.duration} months membership
                    </p>
                  </div>
                  <div className="flex justify-between gap-[20px]">
                    <div>
                      <h3 className="font-manrope text-[24px] font-bold text-[#FFF] mb-[8px]">
                        {member.title}
                      </h3>
                      <ul className="flex gap-[15px]">
                        <li className="text-[#FFF] font-manrope text-[18px] font-medium border-r border-[#FFF] pr-[10px] leading-[15px]">
                          {member.session} sessions
                        </li>
                        <li className="text-[#FFF] font-manrope text-[18px] font-medium pr-[10px] leading-[15px] ">
                          1 service
                        </li>
                      </ul>
                    </div>
                    <div>
                      <h2 className="text-[#FFF] font-manrope text-[28px] font-bold mb-[4px]">
                        SAR {member.sar}
                      </h2>
                      <button className="rounded-[4px] py-[4px] px-[16px] bg-white/80 text-primary  font-manrope font-semibold text-[14px]">
                        Save {member.save}%
                      </button>
                    </div>
                  </div>
        ...
        <div className="flex-shrink-0 w-[400px] border border-primary rounded-[16px] p-[30px] h-max">
          ...
          <div className="flex flex-col gap-[64px]">
            ...
            <div className="border py-[30px] border-primary border-b-borderColorLight border-l-0 border-r-0">
              <p className="text-[#545454] font-manrope text-[18px] font-medium">
                No membership card selected yet
              </p>
            </div>
